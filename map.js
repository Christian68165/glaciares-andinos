...
// 💡 Mejora: Usar 'const' en lugar de reasignar variables globales si no cambian.
const mapboxgl.accessToken = 'pk.eyJ1IjoidXNvcGRldiIsImEiOiJjbWd2ZW1ubGkwcW5xMm5uYXhtb2ptZHF4In0.OE8nb_G4PE0_PduKWdjunw';
...
        color: "#FFA500", 
        tipo: "línea" // ⚠️ Asumo 'rios' es Linea, no Poligono (ajusta si es Poligono)
    },
    "Límites de Cuenca": {
        url: "URL_STORAGE_CUENCA", // ❗ Importante: URL DE RELLENO
        color: "#4682B4", 
        tipo: "polígono"
    },
...
    };
    
    // 🛑 FUNCIÓN ACTUALIZADA: Renderiza foto específica o galerías genéricas (MEJORA: Validación de URL)
    const renderContent = (type, url) => {
        // Validación de URL más estricta para seguridad y evitar errores
        const isValidUrl = url && typeof url === 'string' && url.trim().length > 5 && (url.startsWith('http') || url.startsWith('data:'));
        
        if (type === 'specific' && isValidUrl) {
            // Renderiza una sola foto (ej. Zonas Turísticas)
            return `<div style="max-height: 150px; overflow: hidden; margin-bottom: 10px; border-radius: 4px; border: 1px solid #ddd;"><img src="${url.trim()}" alt="Foto" style="width: 100%; height: auto; display: block; object-fit: cover;"></div>`;
        }
        
...
                photoHTML += `<img src="${fotoUrl}" alt="Pasivo Minero" style="min-width: 100px; height: 75px; object-fit: cover; border-radius: 4px; cursor: pointer;" onclick="window.open('${fotoUrl}', '_blank')">`;
            });
            photoHTML += '</div>';
            return photoHTML;
        }
...
// =================================================================
// 🛑 FUNCIÓN: Lógica para el control 2D/3D (Mantenida)
// =================================================================
...
            is3DView = true;
        }
        // 💡 MEJORA: Podrías añadir lógica aquí para re-aplicar estilos 2D/3D a las vistas dinámicas si es necesario.
    });
}


// =================================================================
// 🛑 NUEVAS FUNCIONES: Lógica de Carga y Control para GeoJSON Fijo (Optimizado)
// =================================================================
...
    };
    const mapboxType = typeMap[geomType] || 'fill'; // Fallback a 'fill' para polígonos

    if (mapboxType === 'circle') {
        return {
            'circle-radius': 8, 
            'circle-color': color, 
...
        };
    } else if (mapboxType === 'line') {
        return {
            'line-color': color, 
            'line-width': 4,
            'line-opacity': opacity
        };
    } else { // fill (Polygon, MultiPolygon)
        return {
            'fill-color': color, 
...
    
    // 🛑 CORRECCIÓN CLAVE: Evitar cargar si la URL es el placeholder o inválida
    if (layerConfig.url === 'URL_STORAGE_CUENCA' || !layerConfig.url.startsWith('http')) {
        setLegendStatus(`Advertencia: URL de ${layerKey} no está definida o es inválida.`);
...
            
            // 🛑 MEJORA: Validar estructura GeoJSON
            if (!geojson || geojson.type !== 'FeatureCollection') {
                throw new Error("Datos GeoJSON inválidos.");
            }

            // Determinar el tipo de geometría Mapbox. Se basa en el tipo configurado, no en el detectado automáticamente.
...
                // Usamos el 'tipo' configurado para el estilo
                map.addLayer({
                    id: layerId,
                    type: mapboxGeomType,
                    source: sourceId,
...
                        // 💡 MEJORA: Evitar mostrar propiedades "vacías" o grandes
                        if (prop === 'description' && !feat.properties[prop]) continue;
                        if (feat.properties[prop] !== null && feat.properties[prop] !== undefined && feat.properties[prop] !== '') {
                            popupHTML += `<p><strong>${prop}:</strong> ${feat.properties[prop]}</p>`;
                        }
                    }
                    
                    // Determina la posición del popup
                    const popupLngLat = mapboxGeomType === 'circle' ? e.lngLat : e.features[0].geometry.coordinates[0][0]; // Usa el primer punto del polígono/línea si no es punto
                    
                    new mapboxgl.Popup()
                        .setLngLat(e.lngLat)
                        .setHTML(popupHTML)
...
    if (!container) return;
    // 💡 MEJORA: No limpiar todo, solo agregar las nuevas capas GeoJSON debajo de las dinámicas
    // Si no tienes un contenedor separado para las GeoJSON fijas, mantén esta función.
    // container.innerHTML = ''; // Comentado para no borrar las dinámicas (si usan el mismo contenedor)
...
// ************ FUNCIONES DE CARGA DE DATOS (Mantenida y robustecida) ************
...
    // 💡 MEJORA: El filtro 'select=*' es implícito, pero no está de más.
...
            if (!f.geom) continue; 
            try {
                // 💡 MEJORA: Aceptar 'geom' como string o como objeto JSON directamente
...
                if (!geomType) geomType = geometry.type;
            } catch (err) { console.warn('GeoJSON inválido en fila', f, err); }
        }
        const geojson = { type: "FeatureCollection", features };
        
        // Usa el tipo detectado (geomType) para la capa.
...
        // Si la capa ya existe, la eliminamos para recrearla con el estilo correcto (2D/3D)
        if (map.getLayer(layerId)) { map.removeLayer(layerId); }

        const beforeId = 'sky'; 

        // Lógica de 2D/3D (Mantenida la extrusión simple para Lagunas)
...
// ************ FUNCIONES DE HANDLERS DEL MENÚ (Mantenidas) ************
